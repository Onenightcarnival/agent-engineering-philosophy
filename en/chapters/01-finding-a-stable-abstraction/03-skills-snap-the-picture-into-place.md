...
A skill is easy to mistake for yet another tool. But they are not the same kind of thing. A tool is a single call, discarded after use; it doesn't follow the agent into the next conversation. A skill has a name, can be reused, and can be moved from one agent to another without losing much. A nameable, reusable, transferable unit of capability — exactly like a skill that lives in a person. Someone who can make slide decks can still make slide decks at the next company.
...
The system prompt lands as the JD. What it writes down is exactly what this role does, who it reports to, what it may and may not touch. It's a statement of job responsibilities, not an incantation to be endlessly re-tuned.
...
Parts matching labels counts for little on its own — anyone can force a metaphor. What makes this view hold is that the relationships between the parts match too. The JD constrains which skills this person should use, rather than letting them unleash everything on the resume. The degree decides which tier of work to hand over. The record travels with the person and survives a transfer. These relationships weren't bolted on after the fact. Put the "person" abstraction in place, and they surface by themselves.
